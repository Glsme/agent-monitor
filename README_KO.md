...
- **커스텀 에이전트 색상** — 오피스에서 에이전트별 색상을 지정하면 대시보드 카드에도 즉시 반영되며, 로컬에 저장되어 재시작해도 유지됩니다
- **멀티팀 지원** — 드롭다운으로 팀 전환
...
| **에이전트 색상** | 오피스에서 변경한 색상이 대시보드 카드와 즉시 동기화됩니다 |
...
에이전트 캐릭터를 클릭하면 하단 패널에 상세 정보가 표시되며, 필요 시 색상 변경도 할 수 있습니다.
